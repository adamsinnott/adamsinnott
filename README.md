# Adam Sinnott

Senior React Native Engineer | AI Workflow Architect | Founder of AppCreators and Droxford AI

I build production-grade mobile apps and AI-powered systems for startups and SMEs.

---

## 🚀 What I Do

- 📱 React Native (Expo, Ignite, MobX, Zustand)
- ☁️ Supabase (Postgres, RLS, Edge Functions)
- 🔄 n8n Agentic AI Workflows
- 🤖 RAG / CAG Architectures
- 🔐 Secure JWT & OAuth Proxies
- 🧠 AI Tooling (LangChain, vector DBs, embeddings)
- 🌍 Local AI Assistants (Droxford AI)

---

## 🧩 Current Focus

- Advanced AI agentic workflows
- Cache-Augmented Retrieval (CAG)
- Production-ready Supabase architectures
- AI-enabled local knowledge systems

---

## 🛠 Tech Stack

**Mobile**
- React Native
- React Navigation
- Expo Router
- TypeScript
- Zustand
- React Native Paper

**Backend**
- Supabase
- PostgreSQL
- Cloudflare Workers
- Node.js

**AI**
- OpenAI
- Gemini
- Vector Databases
- n8n
- Embeddings pipelines

---

## 🏗 Featured Projects

### Droxford AI
Local AI assistant integrating:
- Parish Council documents
- Historical archives
- Bin schedules
- Community knowledge

Built with React Native + Supabase + n8n.

---

### AppCreators
Custom web & mobile app development for SMEs.

https://appcreators.co.uk

---

## 📫 Contact

- Website: https://appcreators.co.uk/contact

---

## ⚙️ Philosophy

Build tools that:
- Are secure by design
- Solve real problems
- Scale cleanly
- Respect users

---

> Engineering should remove friction, not create it.
